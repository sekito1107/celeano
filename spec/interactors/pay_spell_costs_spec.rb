...
RSpec.describe PaySpellCosts do
  ...
  context "when moves include spells and units" do
    let(:spell_card) { create(:card, card_type: :spell, cost: "2") } # Min 2
    let(:unit_card) { create(:card, card_type: :unit, cost: "3") } # Min 3
    ...
    it "pays only spell costs silently and records pending costs" do
      # Expect SAN to decrease by spell cost (2) only
      result = nil
      expect {
        result = described_class.call(game: game)
      }.to change { game_player.reload.san }.by(-2)
      ...
      expect(result.pending_costs.values).to include(
        a_hash_including(amount: 2, user_id: user.id)
      )
    end
  end
end
